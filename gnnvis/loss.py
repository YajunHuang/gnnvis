...
class UMAPLoss(nn.Module):
    ...
    def q_dis(self, dis):
        return 1.0 / (1.0 + self._a * torch.pow(dis, self._b))
    ...
    def t_student_dis(self, scores, n):
        ...
        D2 = torch.add(torch.add(-2 * torch.mm(scores, scores.t()), sum_X).t(),
                       sum_X)  # np.dot(X,X.T)
        q_dist = self.q_dis(D2)
        sum_t_dist = (torch.sum(q_dist) - n)
        Q = torch.div(q_dist - I, sum_t_dist) + I
        Q = torch.max(Q, torch.zeros_like(Q).fill_(1e-12))
        return Q
    ...
    def forward(self, logits, P, mamap=None, mimap=None, neg_sample_ratio=1.0):
        n = len(logits)
        Q = self.t_student_dis(logits, n)
        loss_train = self.CrossEntropy(P, Q, n)
        if mamap is not None or mimap is not None:
            loss_train = neg_sample_ratio * loss_train.mul(mimap) + loss_train + (
                    loss_train.mul(mamap) / neg_sample_ratio)
        return loss_train.sum()


class LargeVisLoss(nn.Module):
    def __init__(self, iscuda, n_neg=None, batch_size=None, pos_times=20, neg_times=1):
    ...
    def forward(self, logits, P, n_neg=20, gamma=7.0):
        pos_P = P > torch.min(P)
        neg_P = P <= torch.min(P)

        sum_X = torch.sum(torch.pow(logits, 2), 1)
        D2 = torch.add(torch.add(-2 * torch.mm(logits, logits.t()), sum_X).t(),
                       sum_X)  # np.dot(X,X.T)

        p_d2 = self.f1(D2, 0.25)

        p_pos = torch.clamp(p_d2[pos_P], 1e-12, 1 - 1e-12)
        p_neg = torch.clamp(p_d2[neg_P], 1e-12, 0.99)
        ...
        loss = torch.sum(gamma * p_neg)
        loss += torch.sum(p_pos)
...
class UMAPNEGLoss(nn.Module):
    ...
    def forward(self, logits, xs, ys, y_neg, sample_edge_weight, n_neg=20, gamma=7.0):
        ...
        p_neg = (sample_edge_weight - 1).expand((self.n_neg, self.batch_size)).T * torch.log(
            torch.div(p_neg - 1, (sample_edge_weight - 1).expand((self.n_neg, self.batch_size)).T))
...
class LargeVisNEGLoss(nn.Module):
    def __init__(self, iscuda, n_neg, batch_size, pos_times=20, neg_times=1):
        super(LargeVisNEGLoss, self).__init__()
        self.iscuda = iscuda
        self.batch_size = batch_size
        self.n_neg = n_neg
        self.pos_times = pos_times
