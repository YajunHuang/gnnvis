"""Dataset utilities, copy from dgl v0.4"""
...
__all__ = ['loadtxt','download', 'check_sha1', 'extract_archive',
           'get_download_dir', 'Subset', 'split_dataset', 'make_split_masks']
...
def make_split_masks(num_samples, split_rates):
    """ Create data split masks

    Parameters:
    -----------
    split_rates: list
        The percentages of data spliting.

    Returns:
    --------
    """
    N = num_samples
    split_rates = [0.0] + split_rates
    sum_rates = float(sum(split_rates))
    if sum_rates != 1.0:
        split_rates = [x / sum_rates for x in split_rates]
    split_percentiles = [split_rates[0]]
    for i in range(1, len(split_rates)):
        split_percentiles.append(split_percentiles[-1] + split_rates[i])
    split_masks = []
    for i in range(len(split_percentiles) - 1):
        mask = np.ones(N).astype(int)
        mask[int(N * split_percentiles[i]) : int(N * split_percentiles[i+1])] = 0
        split_masks.append(mask > 0)
    return split_masks
...
def extract_archive(file, target_dir):
    ...
    if os.path.exists(target_dir):
        return
    # tarfile package reports "tarfile.ReadError: file could not be opened successfully",
    # so gzip package is used here and target_dir is the output file path.
    if file.endswith('.gz'):
        output_file = target_dir
        g_file = gzip.GzipFile(file)
        with open(output_file, 'wb') as fh:
            fh.write(g_file.read())
        g_file.close()
        return
